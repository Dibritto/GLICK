...
import { TrendingUp, TrendingDown, Activity, Zap, Clock, ShieldCheck, ListFilter, BarChart3 } from 'lucide-react';
...
const MainConsole: React.FC<MainConsoleProps> = ({ 
  ...
  if (activeView === 'movimentacoes') {
    return <MovementsView onAddTransaction={() => onOpenTransactionModal()} />;
  }

  if (activeView === 'receitas') {
    return <MovementsView typeFilter="in" title="Gestão de Receitas" onAddTransaction={() => onOpenTransactionModal('income')} />;
  }

  if (activeView === 'despesas') {
    return <MovementsView typeFilter="out" title="Gestão de Despesas" onAddTransaction={() => onOpenTransactionModal('expense')} />;
  }

  if (activeView === 'contas') {
    return <AccountsView />;
  }
    ...
    <main className="flex-1 p-4 md:p-8 space-y-8">
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="glass-panel technical-border p-5 rounded-xl glow-blue-hover transition-all">
            ...
            <p className="text-2xl font-mono font-bold text-white">R$ 3.650,50</p>
          ...
          <div className="glass-panel technical-border p-5 rounded-xl glow-blue-hover transition-all">
            ...
            <p className="text-2xl font-mono font-bold text-brand-green">R$ 1.200,00</p>
          ...
          <div className="glass-panel technical-border p-5 rounded-xl glow-blue-hover transition-all">
            ...
            <p className="text-2xl font-mono font-bold text-brand-red">R$ 2.550,00</p>
            <div className="mt-4 flex items-center gap-2 text-[10px] text-gray-500">
          ...
          <div className="bg-brand-blue/10 border border-brand-blue/30 p-5 rounded-xl glow-blue">
            <p className="text-[10px] uppercase tracking-widest text-brand-blue mb-2">Saldo Final Previsto</p>
            <p className="text-2xl font-mono font-bold text-brand-blue">R$ 2.300,50</p>
            <div className="mt-4 flex items-center gap-2 text-[10px] text-brand-blue/70">
              <ShieldCheck size={12} />
              <span>Margem de segurança: R$ 500</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        <section className="space-y-4">
          ...
          <div className="glass-panel technical-border p-6 rounded-xl h-[300px]">
        ...
        <section className="space-y-4">
          ...
          <div className="glass-panel technical-border p-6 rounded-xl space-y-6 h-[300px] flex flex-col justify-center">
            <div className="space-y-2">
              <div className="flex justify-between items-end">
                ...
                <p className="text-sm font-mono font-bold text-brand-red">R$ 1.800,00</p>
              </div>
              <div className="h-2 w-full bg-brand-lead/20 rounded-full overflow-hidden">
                <motion.div initial={{ width: 0 }} animate={{ width: '50%' }} className="h-full bg-brand-red" />
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-end">
                ...
                <p className="text-sm font-mono font-bold text-brand-orange">R$ 1.000,00</p>
              </div>
              <div className="h-2 w-full bg-brand-lead/20 rounded-full overflow-hidden">
                <motion.div initial={{ width: 0 }} animate={{ width: '28%' }} className="h-full bg-brand-orange" />
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-end">
                ...
                <p className="text-sm font-mono font-bold text-brand-green">R$ 850,50</p>
              </div>
              <div className="h-2 w-full bg-brand-lead/20 rounded-full overflow-hidden">
                <motion.div initial={{ width: 0 }} animate={{ width: '22%' }} className="h-full bg-brand-green" />
              </div>
            </div>
          </div>
        </section>
      ...
      <section className="space-y-4">
        ...
        <div className="glass-panel technical-border rounded-xl overflow-hidden overflow-x-auto no-scrollbar">
          <table className="w-full text-left border-collapse min-w-[600px]">
            ...
            <tbody className="divide-y divide-brand-lead/20">
              {[
                { date: '14 Mar', desc: 'Posto Shell', cat: 'Transporte', acc: 'Itaú', val: '- R$ 85,50', type: 'out' },
                { date: '12 Mar', desc: 'Supermercado', cat: 'Alimentação', acc: 'Nubank', val: '- R$ 150,00', type: 'out' },
                { date: '10 Mar', desc: 'Aluguel', cat: 'Moradia', acc: 'Nubank', val: '- R$ 1.200,00', type: 'out' },
                { date: '05 Mar', desc: 'Salário Mensal', cat: 'Renda', acc: 'Nubank', val: '+ R$ 5.000,00', type: 'in' },
              ].map((t, i) => (
                <tr key={i} className="hover:bg-brand-lead/10 transition-colors group cursor-pointer">
                  <td className="px-6 py-4 text-xs font-mono text-gray-400">{t.date}</td>
                  <td className="px-6 py-4 text-xs font-bold text-gray-200">{t.desc}</td>
                  <td className="px-6 py-4">
                    <span className="px-2 py-0.5 rounded bg-brand-lead/30 text-[10px] text-gray-400">{t.cat}</span>
                  </td>
                  <td className="px-6 py-4 text-xs text-gray-500">{t.acc}</td>
                  <td className={`px-6 py-4 text-xs font-mono font-bold text-right ${t.type === 'in' ? 'text-brand-green' : 'text-brand-red'}`}>
                    {t.val}
                  </td>
                </tr>
              ))}
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-12">
        <div className="glass-panel technical-border p-6 rounded-xl flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-3xl font-mono font-bold text-white">R$ 95,00</p>
        ...
        <div className="glass-panel technical-border p-6 rounded-xl flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-3xl font-mono font-bold text-brand-green">18 Dias</p>
            <p className="text-[10px] text-gray-400 mt-1">Seu dinheiro dura até 02/04</p>
          </div>
        </div>
      </section>
    </main>
