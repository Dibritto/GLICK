...
const QuickActions: React.FC<QuickActionsProps> = ({ 
  ...
  return (
    <div className="fixed bottom-4 lg:bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-1 lg:gap-2 p-1.5 bg-brand-gray-deep/95 border border-brand-lead/50 rounded-full backdrop-blur-md shadow-2xl z-50 max-w-[95vw] overflow-x-auto no-scrollbar">
      <button 
        onClick={onAddIncome}
        className="flex items-center gap-2 px-4 lg:px-6 py-2.5 bg-brand-green/10 text-brand-green hover:bg-brand-green/20 border border-brand-green/20 rounded-full transition-all group shrink-0"
      >
      ...
      <button 
        ...
        className="flex items-center gap-2 px-4 lg:px-6 py-2.5 bg-brand-red/10 text-brand-red hover:bg-brand-red/20 border border-brand-red/20 rounded-full transition-all group shrink-0"
      >
